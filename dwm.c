...
void drawbar(Monitor *m) {
  ...
  Client *c;
  drw_funcs statusbar[3] = {sb_xmonad, sb_roundcorner, sb_powerline};
  int n = -1, index = 0;
  const char *delim = ",";
  ...
  if (m == selmon) { /* status is only drawn on selected monitor */
    ...
    statusbar[which_sb % LENGTH(statusbar)](m->ww - tw, &tw, n, index, info);
  }
  status_range = tw;
  resizebarwin(m);
  for (c = m->clients; c; c = c->next) {
...
void sb_powerline(int x, int *tw, int n, int index, const char **info) {
  int dxdy = bh / 2, w;
  while (n >= 0) {
    if (index != 0) {
      drw_setscheme(
          drw,
          sb_scheme[(colour_shift ? counter : index) % LENGTH(sb_colours)]);
...
void sb_roundcorner(int x, int *tw, int n, int index, const char **info) {
  int w, wt;
  int wc = TEXTW("") - lrpad;
  while (n >= 0) {
    if (index != 0) {
...
void sb_xmonad(int x, int *tw, int n, int index, const char **info) {
  int w;
  x -= right_padding;
...
void sb_scroll(const Arg *arg) {
  which_sb++;
  drawbar(selmon);
}

void sb_shiftcolour(const Arg *arg) {
  colour_shift ^= 1;
  drawbar(selmon);
}
